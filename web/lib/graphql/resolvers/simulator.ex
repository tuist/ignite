defmodule IgniteWeb.GraphQL.Resolvers.Simulator do
  ...
  def list_simulators(_parent, _args, _resolution) do
    # Get simulators from Sidekick
    case GenServer.call(Ignite.Sidekick, :list_simulators) do
      {:ok, simulators} ->
  ...
  def list_devices(_parent, _args, _resolution) do
    # Get devices from Sidekick
    case GenServer.call(Ignite.Sidekick, :list_devices) do
      {:ok, devices} ->
        devices = devices
          |> Enum.map(fn dev ->
            %{
